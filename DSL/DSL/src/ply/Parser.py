...
class Parser:
    ...
    def p_job(self, p):
        ...
        if len(p) == 2:
            ...
            p[0] = SyntaxTree(('root'), p[2], *p[3].childs)

    def p_stepdecl(self, p):
        '''
        stepdecl : STEP ID expressions ENDSTEP
        '''
        p[0] = SyntaxTree(('stepdecl', p[2]), *p[3])
    ...
    def p_oneline(self, p):
        ...
        '''
        if len(p) == 4 and p[2] == '=':
            p[0] = SyntaxTree(('expression', 'assign', p[1]), p[3])
        else:
            p[0] = SyntaxTree(('expression', p[1]), *p[2:])

    def p_switch(self, p):
        '''
        switch : SWITCH VAR  switch_body ENDSWITCH
        '''
        p[0] = SyntaxTree(('expression', 'switch', p[2]), *p[3])
        pass
    ...
    def p_default(self, p):
        ...
        '''
        p[0] = [SyntaxTree(('default',), p[2])]

    def p_case(self, p):
        '''
        case    : CASE STR oneline
        '''
        p[0] = SyntaxTree(('case', p[2]), p[3])
    ...
    def p_term_var(self, p):
        ...
        term    : VAR 
        '''
        p[0] = SyntaxTree(("var", p[1]))

    def p_term_str(self, p):
        '''
        term    : STR
        '''
        p[0] = SyntaxTree(("str", p[1]))
